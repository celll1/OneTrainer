...
class BasePixArtAlphaSetup(
    ...
    def setup_optimizations(
        ...
        target_module = getattr(model, 'transformer', None) # Target Transformer for PixArt

        if target_module is not None:
            if attn_processor_type == AttentionProcessorType.SAGE.value:
                if SAGE_ATTENTION_AVAILABLE:
                    print("Setting SageAttentionProcessor for Transformer...")
                    try:
                        # Use the standard SD/SDXL processor for PixArt's Transformer
                ...
                if FLASH_ATTENTION_AVAILABLE:
                    print("Setting FlashAttention2Processor for Transformer...")
                    try:
                        # Use the standard SD/SDXL processor for PixArt's Transformer
                        processor = FlashAttention2Processor()
                        target_module.set_attn_processor(processor)
                        print("Successfully set FlashAttention2Processor for Transformer.")
                    except Exception as e:
